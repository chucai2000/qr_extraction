...
def utility_contains(one_contour, another_contour):
	...
	for ptIndx in range(another_contour.shape[0]):
		pt = (another_contour[ptIndx,0],another_contour[ptIndx,1])
	...
	another_contour_side_length = np.linalg.norm(np.vstack((np.diff(another_contour, axis=0), another_contour[0,:]-another_contour[another_contour.shape[0]-1,:])), axis=1)
...
def pre_process_img(img):
	...
	img_binarize = cv2.adaptiveThreshold(img_gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 2*BLOCK_RADIUS+1, 0)
	...
	for i in range(1,height):	
		...
		while i - k >= 0 and k <= PROBE_LEN and nIncr > 0:
			if (utility_get_norm_dist(img[i,0],img[i-k,0]) > EFFECTIVE_COLOR_DIFF):
				img_integral[i,0] = img_integral[i,0] + INCR
				nIncr = nIncr - 1
			k = k + 1
...
def get_connected_components_from_block_mask(block_mask):
	...
	""" Perform morphological processing over the block mask map, to 1) remedy the holes within the foreground connected components; 2) remove the tiny noise """
	...
	contours, hierarchy = cv2.findContours(block_mask_morph_copy, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
...
def clear_nested_contours(img_patch, contours):
	...
	""" Indicate whether a contour should be preserved or removed 
		check_map element is a list in format: [contour_idx, contain_contours, reside_contained_by_contours, nest_contained_by_contours]	
	"""
	if not contours:
...
def estimate_a_third_contour_centroid(centroid1, centroid2):
	...
	centroid0_homo = np.cross(centroid_diff_vec,ancillary) / (centroid_distance/2) + (centroid1_homo+centroid2_homo)*0.5
	centroid0_estimates.append((centroid0_homo[0:2]+0.5).astype(np.int32))
	centroid0_homo = np.cross(ancillary,centroid_diff_vec) / (centroid_distance/2) + (centroid1_homo+centroid2_homo)*0.5
	centroid0_estimates.append((centroid0_homo[0:2]+0.5).astype(np.int32))
		...
		if ct0_est[0] > 0 and ct0_est[1] > 0:
			for i in range(len(contours)):
				...
				if np.fabs(area1-area0) <= np.amin([area1,area0])*VERIFY_GROUPED_SQUARE_AREA_DIFF_TO_MINIMUM_RATIO and \
					...
					if (abs(ct0_est[0]-ct0[0]) <= VERIFY_ESTIMATED_SQUARE_DISTANCE and abs(ct0_est[1]-ct0[1]) <= VERIFY_ESTIMATED_SQUARE_DISTANCE) or \
						((abs(ct0_est[0]-ct0[0]) <= VERIFY_ESTIMATED_SQUARE_DISTANCE or abs(ct0_est[1]-ct0[1]) <= VERIFY_ESTIMATED_SQUARE_DISTANCE) and 
...
def extract_grouped_squared_finder_patterns(img_patch, contours, contours_nest_external, check_map):
	...
	if num_contours_nest_external == 0:
		...
		if centroid0_truth_indx != -1:
			...
			if np.amax([side10,side11,side12,side13]) <= FINDER_PATTERN_SIDE_LENGTH_MAXIMUM and np.amax([side20,side21,side22,side23]) <= FINDER_PATTERN_SIDE_LENGTH_MAXIMUM:
				return 'Probable', contours_nest_external
			else:
				return 'False', contours_nest_external
...
def get_finder_patterns_in_hypotheses(img, bounding_rects_hypotheses,is_pop_display):
	...
	for br in bounding_rects_hypotheses:
		...
		img_edge = cv2.Canny(img_patch_gray, CANNY_LOW_THRESHOLD, CANNY_HIGH_THRESHOLD, L2gradient=True); img_edge_copy = img_edge.copy()
		contours, hierarchy = cv2.findContours(img_edge_copy, cv2.RETR_TREE, cv2.CHAIN_APPROX_NONE)	
		...
		for ct in contours_quad:
			...
			# angle between neighoring sides may be useful
		...
		contours_quad_nest_clear, contours_quad_nest_clear_external, check_map = clear_nested_contours(img_patch, contours_quad_clear)
		print("Length of cleared set of contours and external nest contours: " + str(len(contours_quad_nest_clear)) + " , " + str(len(contours_quad_nest_clear_external)))

		""" Fix and Verify the squares of finder patterns to extract true positive qr-code patches """
		is_containing_qr_code, contours_finalize = extract_grouped_squared_finder_patterns(img_patch, contours_quad_nest_clear, contours_quad_nest_clear_external, check_map)
		...
		if is_pop_display:
			...
			img_displays = (img_patch_dst_quad,img_patch_dst_quad_clear,img_patch_dst_quad_nest_clear,img_patch_dst_quad_nest_clear_external)
		...
		result = {'hypothesis_bounding_rect': [np.array([ [br[0],br[1]],[br[0]+br[2]-1,br[1]],[br[0]+br[2]-1,br[1]+br[3]-1],[br[0],br[1]+br[3]-1] ])], 
				'img_patch':img_patch, 'finder_patterns':contours_finalize_offset, 'is_containing_qr_code':is_containing_qr_code}
...
def displays2(img, rects,  color=(255,255,0)):
	...
	img_dst = img.copy()
	contours = [np.array([ [r[0],r[1]],[r[0]+r[2]-1,r[1]],[r[0]+r[2]-1,r[1]+r[3]-1],[r[0],r[1]+r[3]-1] ]) for r in rects]
